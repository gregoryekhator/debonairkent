 ...
 * @return string
 */
function theme_university_frontpage_slideshow() {
    global $PAGE;
    $slidetoggle = theme_university_get_setting("toggleslideshow");
    if (!$slidetoggle) {
        return '';
    }
    $numberofslides = theme_university_get_setting('numberofslides');
    $slideimage = '';
    $visableslide = 0;

    for ($s1 = 1; $s1 <= $numberofslides; $s1++) {
        $slideimage .= theme_university_render_slideimg($s1, 'slide' . $s1 . 'image');
    }
    ...
    if ($slideimage) {
        ...
        if ($numberofslides) {
            ...
            for ($s1 = 1; $s1 <= $numberofslides; $s1++):
                $slidecaption = theme_university_get_setting('slide'.$s1.'caption', true);
                $slidecaption = theme_university_lang($slidecaption);
                $slideimg = theme_university_render_slideimg($s1, 'slide' . $s1 . 'image');
                if (empty($slideimg)) {
                    $slideimg = '';
                }
                $slidebtn = theme_university_get_setting('slide'.$s1.'urltext');
                $slidebtn = theme_university_lang($slidebtn);
                $slidebtnurl = theme_university_get_setting('slide' . $s1 . 'url');
                $icon = "fa-angle-right";
                if (right_to_left()) {
                    $icon = "fa-angle-left";
                }
