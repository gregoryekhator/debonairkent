 ...
 * @return null
 */
function theme_university_page_init(moodle_page $page) {
    $page->requires->jquery();
    $page->requires->js('/theme/university/javascript/theme.js');
}
 ...
 * @return string $css
 */
function theme_university_process_css($css, $theme) {
    ...
    $css = theme_university_set_fontwww($css);
    $css = theme_university_get_pattern_color($css, $theme);
    $css = theme_university_set_customcss($css, $customcss);
 ...
 * @return bool
 */
function theme_university_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;

    if (empty($theme)) {
        $theme = theme_config::load('university');
    }
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        if ($filearea === 'logo') {
            ...
            theme_university_serve_css($args[1]);
 ...
 * @return string
 */
function theme_university_serve_css($filename) {
    global $CFG;
    if (!empty($CFG->themedir)) {
        $thestylepath = $CFG->themedir . '/university/style/';
    } else {
        $thestylepath = $CFG->dirroot . '/theme/university/style/';
    }
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_university_send_unmodified($lastmodified, $etagfile);
    }
    theme_university_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}
 ...
 *
 */
function theme_university_send_unmodified($lastmodified, $etag) {
 ...
 * @param string $etag
 */
function theme_university_send_cached_css($path, $filename, $lastmodified, $etag) {
    global $CFG;
 ...
 */
function theme_university_set_customcss($css, $customcss) {
 ...
 */
function theme_university_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
 ...
 */
function theme_university_set_fontwww($css) {
    global $CFG, $PAGE;
    ...
    $tag = '[[setting:fontwww]]';
    $theme = theme_config::load('university');
    $css = str_replace($tag, $themewww.'/university/fonts/', $css);
    return $css;
}
 ...
 * @return image $logo
 */
function theme_university_get_logo_url($type='header') {
    global $OUTPUT;
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('university');
 ...
 * @return null
 */
function theme_university_render_slideimg($p, $sliname) {
    global $PAGE, $OUTPUT;

    $nos = theme_university_get_setting('numberofslides');
    $i = $p % 3;
    // Get slide image or fallback to default.
    if (theme_university_get_setting($sliname)) {
 ...
 * @return bool
 */
function theme_university_get_setting($setting, $format = true) {
    global $CFG;
 ...
 * @return string
 */
function theme_university_theme_url() {
    global $CFG, $PAGE;
 ...
 */
function theme_university_generate_links($menuname = '') {
    global $CFG, $PAGE;
    $htmlstr = '';
    $menustr = theme_university_get_setting($menuname);
    ...
    foreach ($menusettings as $menukey => $menuval) {
        ...
        if (!empty($expset) && isset($expset[0]) && isset($expset[1])) {
            ...
            $ltxt = trim($ltxt);
            $ltxt = theme_university_lang($ltxt);
            $lurl = trim($lurl);
            if (empty($ltxt)) {
                continue;
            }
 ...
 * @return array
 */
function theme_university_hidden_courses_ids() {
    global $DB;
    $hcourseids = array();
 ...
 * @return string
 */
function theme_university_strip_html_tags( $text ) {
    $text = preg_replace(
        array(
 ...
 * @return string $out
 */
function theme_university_course_trim_char($str, $n = 500, $endchar = '&#8230;') {
    if (strlen($str) < $n) {
        return $str;
    }
 ...
 * @return string
 */
function theme_university_lang($key = '') {
    $pos = strpos($key, 'lang:');
    if ($pos !== false) {
        list($l, $k) = explode(":", $key);
        if (get_string_manager()->string_exists($k, 'theme_university')) {
            $v = get_string($k, 'theme_university');
            return $v;
        } else {
            return $key;
        }
 ...
 * @return string
 */
function theme_university_get_pattern_color( $css, $type='') {
    global $OUTPUT , $CFG;
    ...
    $patterncolors = array (
        'default' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/university/pix/default/t/collapsed_empty',
            'collapsed' => $CFG->wwwroot.'/theme/university/pix/default/t/collapsed',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/university/pix/default/t/collapsed_rtl',
            'expanded' => $CFG->wwwroot.'/theme/university/pix/default/t/expanded'
        ),
        ...
        '1' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/university/pix/cs01/t/collapsed_empty.png',
            'collapsed' => $CFG->wwwroot.'/theme/university/pix/cs01/t/collapsed.png',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/university/pix/cs01/t/collapsed_rtl.png',
            'expanded' => $CFG->wwwroot.'/theme/university/pix/cs01/t/expanded.png'
        ),
        '2' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/university/pix/cs02/t/collapsed_empty.png',
            'collapsed' => $CFG->wwwroot.'/theme/university/pix/cs02/t/collapsed.png',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/university/pix/cs02/t/collapsed_rtl.png',
            'expanded' => $CFG->wwwroot.'/theme/university/pix/cs02/t/expanded.png'
        ),
        ...
        '3' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/university/pix/cs03/t/collapsed_empty.png',
            'collapsed' => $CFG->wwwroot.'/theme/university/pix/cs03/t/collapsed.png',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/university/pix/cs03/t/collapsed_rtl.png',
            'expanded' => $CFG->wwwroot.'/theme/university/pix/cs03/t/expanded.png'
        ),
        ...
        '4' => array (
            ...
            'collapsed_empty' => $CFG->wwwroot.'/theme/university/pix/cs04/t/collapsed_empty.png',
            'collapsed' => $CFG->wwwroot.'/theme/university/pix/cs04/t/collapsed.png',
            'collapsed_rtl' => $CFG->wwwroot.'/theme/university/pix/cs04/t/collapsed_rtl.png',
            'expanded' => $CFG->wwwroot.'/theme/university/pix/cs04/t/expanded.png'
        )
    );

    $selectedpattern = theme_university_get_setting('patternselect');
 ...
 * @return string
 */
function theme_university_get_hexa($hexa, $opacity) {
    if (!empty($hexa)) {
