...
//used the id to get the course activity that is completed
$records_per=$DB->get_records_sql("select * from {course_modules} where course=$id AND completion=1");

//used the id to get the course activity that is completed or not
$record__per=$DB->get_records_sql("select * from {course_modules} where course=$id");
//Count the result of that activity that are completed
$value = (count($records_per));
//Count the result of that activity that are not completed
$totals = (count($record__per));
//Minus 1 from the result(that is the announement)
$total = $totals -1;
...
echo $categoryname.',dikko,'.$coursename.',dikko,'.$summary.',dikko,'.$percentage;
